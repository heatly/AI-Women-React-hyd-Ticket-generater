    ...
    imgTemplate.onload = () => {
      ...
      reader.onload = (e) => {
        imgUser.onload = () => {
          ...
          while (ctx.measureText(name).width > MAX_TEXT_WIDTH && nameSize > 30) {
            nameSize -= 2;
            ctx.font = getFont(nameSize);
          }
          ...
          for (let i = 1; i < words.length; i++) {
            if (ctx.measureText(currentLine + " " + words[i]).width < MAX_TEXT_WIDTH) {
              currentLine += " " + words[i];
            } else {
          ...
          let currentY = NAME_START_Y;
          lines.forEach(line => {
            ctx.fillText(line.toUpperCase(), NAME_X, currentY);
            currentY += nameSize + 10;
          });
          
          // 4. Draw Designation (Role)
          if (role) {
            let roleSize = Math.max(16, Math.floor(nameSize * 0.45));
            ...
            ctx.fillText(role, NAME_X, currentY);
          }
